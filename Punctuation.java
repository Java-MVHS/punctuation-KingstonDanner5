...
main:
    1. Initialize variable text with the full text.
    2. Call searchIt.

searchIt:
    1. Get punctuation words by calling getPunctuationWords(text).
    2. Print punctuation words by calling printWords.

Method: getPunctuationWords:
    1. Split text into words.
    2. Filter words containing punctuation by calling checkForPunctuation.
    3. Return list of words with punctuation.

Method: checkForPunctuation:
    1. Return true if the word contains punctuation, otherwise false.

Method: printWords:
    1. Print each word in words on the same line.
 */
 ...
 public class Punctuation {
     ...
     // Method to search and print words with punctuation
     public static void searchIt(String text) {
         String[] words = getPunctuationWords(text);
         printWords(words);
     }
 
     // Method to extract words with punctuation
     public static String[] getPunctuationWords(String text) {
         String[] words = text.split(" "); // Split by space
         String[] punctuationWords = new String[words.length];
         int index = 0;
 
         for (String word : words) {
             if (checkForPunctuation(word)) {
                 punctuationWords[index++] = word;
             }
         }
 
         // Trim the array to the actual size
         String[] result = new String[index];
         System.arraycopy(punctuationWords, 0, result, 0, index);
         return result;
     }
 
     // Method to check if a word contains punctuation
     public static boolean checkForPunctuation(String word) {
         return word.matches(".*[\\p{Punct}].*"); // Matches if the word contains any punctuation character
     }
 
     // Method to print the words with punctuation on the same line
     public static void printWords(String[] words) {
         for (String word : words) {
             if (word != null) { // Avoid printing null values
                 System.out.print(word + " "); // Print on the same line with space between words
             }
         }
         System.out.println(); // Move to the next line after all words are printed
     }
 }
